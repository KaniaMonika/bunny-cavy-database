...
root = tk.Tk()
root.title("Change Order Status")
...
def change_status(order_id):
    cursor.execute("UPDATE Orders SET order_status = 'shipped' WHERE order_id = ?", (order_id,))
    conn.commit()
    refresh_orders()

# Create buttons for pending orders
cursor.execute("SELECT order_id FROM Orders WHERE order_status = 'processing'")
for (order_id,) in cursor.fetchall():
    btn = tk.Button(button_frame, text=f"Complete Order {order_id}", width=20, command=lambda oid=order_id: change_status(oid))
